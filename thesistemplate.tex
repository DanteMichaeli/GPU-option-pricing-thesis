...
There are always two parties to every option contract: the investor (buyer), who takes the long position, pays an up front premium to the underwriter (seller) for the right to engage in a future trade. A single option contract is considered a zero-sum game between the investor and the underwriter. Thus, we have a pair of mirrored positions for both a call option and a put option. These can be visualized by graphing the profit of each position at the time of exercising as a function of the underlying asset price. Alternatively, one can consider the slightly modified \textbf{payoff} of the position, which adds the premium back to the profit and focuses on the fundamental mechanics of the option itself:
\begin{equation}
    \text{profit}=\text{payoff}-\text{premium}
\end{equation}

\subsection{Vanilla Option Payoff Structures}


In a similar fashion to \cite[pp. 7-10]{hull2016options}, profit diagrams for each vanilla option position is presented below, along with practical examples for intuition. We consider European-style options for simplicity, but the profit diagrams for American-style options are identical as we only add the option of exercising earlier than at maturity. Transaction costs have been ignored. Let $p$ be the premium, or price paid for the option, $K$ the strike price, and $S_T$ the price of the underlying asset at maturity.



\subsubsection{Call Option Positions}
The investor is willing to pay a premium up front in order to fix a price for a later time. In other words, they expect the price of the underlying asset to rise enough to offset this premium. Figure ~\ref{fig:call_payoff} depicts the profit diagram of a long position of a call option.
...
\centering
    \begin{tikzpicture}[scale=1.5, >=stealth]
        ...
        \fill[gray!10] (2,-0.8) -- (7,-0.8) -- (7,4) -- (2,4) -- cycle;
        ...
        % Add "In the Money" label inside the fully shaded area
        \node[gray!60] at (5,1.5) {\small \textbf{In the Money}};
    \end{tikzpicture}
...
As long as $S_T$ < $K$ we are at a net loss equal to the paid premium $p$. The option is said to be \textbf{out of the money (OTM)}, and has no intrinsic value. When $S_T = K$ the option is considered to be \textbf{at the money (ATM)}, after which, when $S_T > K$ it is \textbf{in the money (ITM)} and has some intrinsic value, or a positive payoff. Still, the profit remains negative as long as payoff < $p$. Only  after offsetting the premium does the option truly become profitable for the investor. Assuming liquid markets, they could then exercise the option to buy the underlying asset for the strike price, only to immediately sell for the current market price to profit off the difference. In general, ITM options are exercised, as the intrinsic value helps to offset at least some of the loss incurred from paying the premium.  
