...
% Path Mode Title
\node[draw=none, font=\bfseries] at (6.5, 9.5) {Path Mode};
% Path Mode Pseudocode
\node[draw=black!30, fill=gray!10, text width=3.5cm, align=left, font=\ttfamily\scriptsize, inner sep=8pt, rounded corners=3pt] at (6.5, 7.5) {
for p = 1 to numPaths \{\\
...
\node[draw=none, font=\normalsize] at (4.5, 5.9) {D*};
% Thread 1 (Blue) - Path 2 (lighter)
\node[rectangle, fill=blue!40, text=white] (b21) at (7, 5.5) {2.1};
...
\node[rectangle, fill=red!40, text=white] (g21) at (7, 4.5) {2.1};
...
% SLICE MODE
\node[draw=none, font=\bfseries] at (6.5, 3) {Slice Mode};
% Slice Mode Pseudocode  
\node[draw=black!30, fill=gray!10, text width=3.5cm, align=left, font=\ttfamily\scriptsize, inner sep=8pt, rounded corners=3pt] at (6.5, 1.5) {
for s = 1 to numSteps \{\\
...
\node[rectangle, fill=red!80, text=white] (sg11) at (0, -1.5) {1.1};
...
\node[draw=none, font=\normalsize] at (10.4, -0.1) {D*};
\end{tikzpicture}
\caption{GPU warp divergence in path mode versus slice mode execution. In path mode (top), threads complete entire paths sequentially, causing divergence at each path boundary when threads have different numbers of steps. In slice mode (bottom), threads process steps in parallel across paths, resulting in only a single divergence point when shorter options complete. Blue represents thread 1 (3 steps), red represents thread 2 (4 steps), with darker shades for path 1 and lighter shades for path 2.}
