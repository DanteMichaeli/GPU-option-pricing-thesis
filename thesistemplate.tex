...
This thesis analyzes the GPU acceleration potential for the CRR and MC option pricing algorithms. The results of the analyzed research are summarized in \cref{tab:results-table}. Due to differences in the experimental setup, including option types, GPU and CPU architectures, and used metrics, direct performance comparisons are inherently difficult. Nevertheless, one can make broader, qualitative comparisons between the methods, as well as draw general conclusions on GPU accelerated option pricing.

An algorithm's inherent dependency structure dictates both its parallelization potential and the nature of such optimizations. The CRR algorithm, which aggregates multiple price paths through a recombining lattice, exhibits rigid dependencies between nodes across time steps. This limits parallelization to computations within each time step. On the other hand, MC methods simulate completely independent price paths, allowing for easier parallelization. This is supported by the data: the naive CRR-GPU implementation achieved an approximately 10x speedup over a single-core CPU \cite{pharr2005gpu}, while the MC-GPU version achieved 23-59x speedups despite being benchmarked against a quad-core CPU \cite{nguyen2007gpu}. Regardless of the method chosen, these straightforward GPU versions yield substantial gains relative to implementation effort, making them a clear improvement over sequential CPU versions. Scalability is eventually limited by bottlenecks -- thread communication latency for CRR and memory constraints for MC, particularly in RNG selection for memory-intensive option types.
...
\hline
\textbf{Algorithm} & \textbf{Reported Speedup} & \textbf{Strategy} & \textbf{Bottleneck} \\
\hline
CRR Naive & 10x\tnote{1} & Parallelization within time periods & Synchronization latency between time steps \\
\hline
CRR Partitioned & 2x\tnote{2} & Additional parallelization across time periods & Inter-partition communication \\
\hline
MC Basic & 23–59x\tnote{3} & Parallel simulation paths & random number generation, memory bandwidth \\
\hline
MC Optimized & 43x\tnote{4} & Memory coalescing, register reuse, warp divergence minimization & Memory bandwidth and register pressure \\
\hline
\end{tabular}
\begin{tablenotes}
\footnotesize
