...
2. For all partitions except the rightmost one, calculate coefficients recursively in parallel, backtracking from the right edge to the left. Since the rightmost partition already has the option values available at its right edge (terminal nodes) we can simply compute the option prices normally here.
...
The key insight is that by dividing the binomial tree into p partitions along the time axis, the algorithm can compute the relationship between partition boundaries in parallel. Each partition independently computes the coefficients that relate option prices at its left boundary to those at its right boundary. Since these coefficient calculations don't depend on each other across partitions, they can be performed simultaneously.
