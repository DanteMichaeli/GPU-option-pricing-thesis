...
\hypersetup{pdfpagemode=UseNone, pdfstartview=FitH,
  pdftitle={GPU-accelerated option pricing algorithms},pdfauthor={Daniel Michaeli},
  pdfkeywords={option pricing, options, gpu, gpu-acceleration, parallel computing, computational finance}}
...
%% by a regular space.
%%
\supervisor{Prof.\ Lauri Savioja} %{Prof.\ Pirjo Professori}
...
%% Avainsanat
%%
\keywords{option pricing, options, GPU, GPU acceleration, parallel computing, computational finance}
%% Abstract text
...
\advisor{DI\ Henrik Lievonen} %
\supervisor{Prof.\ Lauri Savioja}
\degreeprogram{Datateknik}
\department{Högskolan för teknikvetenskaper}%
\professorship{?}  %
%% Abstract keywords
\keywords{optionsprissättning, optioner, GPU, GPU-acceleration, parallell beräkning, computational finance}
%% Abstract text
...
\centering
    \begin{tikzpicture}[scale=1.4, >=stealth]
        ...
        \draw[->] (0,0) -- (6,0) node[below,xshift=-20,yshift=-2pt, align=center] {\small$S_T$ (€)};
    ...
    \end{tikzpicture}
    \caption{Profit diagram for the long position in a call option. $K$ denotes the strike price, \mbox{$S_T$ the spot price} at exercise, and $p$ the premium. The buyer pays a premium to secure the right to purchase the stock at $K$. When $S_T < K$, the option is out of the money (OTM) and expires worthless with profit $-p$. When $S_T > K$, the option is in the money (ITM) and is exercised. The buyer purchases stock at the strike price to sell at the spot price. The position becomes profitable only when $S_T - K > p$.}
    ...
    \begin{tikzpicture}[scale=1.4, >=stealth]
        ...
        \node[gray!60] at (3,-1.5) {\small \textbf{ITM}};
    \end{tikzpicture}
    \caption{Profit diagram for the short position in a call option. $K$ denotes the strike price, $S_T$ the spot price at exercise, and $p$ the premium. The writer receives a premium for the obligation to sell the stock at $K$. When $S_T < K$, the option is out of the money (OTM) and expires worthless. As it is not exercised, the writer retains the full premium as profit. When $S_T > K$, the option is in the money (ITM) and is exercised. The writer is forced to sell the stock at the strike price despite the higher spot price. The profit decreases linearly, becoming negative \mbox{once $S_T - K > p$.}}
...
\begin{tikzpicture}[>=stealth]
    ...
    \draw[->] (0,0) -- (6,0) node[below,xshift=-20,yshift=0pt] {\small$S_T$ (€)};
   ...
   % Draw lattice nodes
    \foreach \i in {0,...,4} {
        \foreach \j in {0,...,\i} {
            ...
            \ifnum\i=4
                \node[circle, draw, minimum size=7mm, fill=gray!60] (n\i\j) at (\i*2,\y) {};
            \else
                \ifnum\i=3
